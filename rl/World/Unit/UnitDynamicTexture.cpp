...
std::vector<float> UnitDynamicTexture::GenNoiseValMap(const float scale) const
{
    ...
    for (; i + simdWidth <= totalPixels; i += simdWidth)
    {
        ...
        float32x4_t indices = vsetq_lane_f32(
            i + 3, vsetq_lane_f32(i + 2,
                vsetq_lane_f32(i + 1,
                    vsetq_lane_f32(i, vdupq_n_f32(0), 0), 1), 2), 3);
                                                    ...
                                                    const std::vector<int>& colorMap) const
{
    ...
    if (index < colorMap.size())
    {
        ...
        float32x4_t base = vsetq_lane_f32(r, vsetq_lane_f32(g, vsetq_lane_f32(b, vdupq_n_f32(0), 0), 1), 2);
        float32x4_t palette = vsetq_lane_f32(pr, vsetq_lane_f32(pg, vsetq_lane_f32(pb, vdupq_n_f32(0), 0), 1), 2);
